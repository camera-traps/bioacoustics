...
taxonomy <-
  read.delim2(
    file = here::here("data", "raw","labels_en.txt"),
    sep = "_",
    header = FALSE,
...
data$timestamp <- basename(data$audio_link)
...
deployments_raw <- data %>%
  ...
  dplyr::mutate(
    ...
    deploymentID = paste0(site, "_", deploymentStart)
    ) %>%
...
media <-
  ...
  dplyr::mutate(
    ...
    gain = NULL,
    channels = "1",
    mediaDuration = "300", # MediaDuration http://rs.tdwg.org/ac/terms/mediaDuration
    exifData = NULL,
    favorite = NULL,
    mediaComments = NULL,
    .keep = "none"
