...
deployments_raw <-
  ...
  dplyr::group_by(.data$site) %>% 
    ...
    deploymentStart = min(detected_time),
    deploymentEnd = max(detected_time_end),
    deploymentID = .data$site,
    ) %>%
  dplyr::select(
    site, latitude, longitude, deploymentStart, deploymentEnd, deploymentID, 
    recorder
    ) %>% 
  dplyr::distinct() %>% 
  dplyr::ungroup()
...
observations <-
  ...
  dplyr::mutate(
    ...
    eventEnd = .data$detected_time_end, # Required
